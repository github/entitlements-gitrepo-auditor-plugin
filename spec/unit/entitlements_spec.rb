...
describe Entitlements do
  ...
  describe "#auditors" do
    ...
    context "with auditors defined" do
      let(:auditor1_cfg) { { "auditor_class" => "GitRepo", "foo" => "bar" } }
      let(:auditor1) { Entitlements::Auditor::GitRepo.new(logger, auditor1_cfg) }
      let(:auditor2_cfg) { { "auditor_class" => "GitRepo", "foo" => "bar" } }
      let(:auditor2) { Entitlements::Auditor::GitRepo.new(logger, auditor2_cfg) }
      let(:entitlements_config_hash) { { "auditors" => [auditor1_cfg, auditor2_cfg] } }

      it "returns an array of auditor objects" do
        allow(Entitlements::Auditor::GitRepo).to receive(:new).with(logger, auditor1_cfg).and_return(auditor1)
        allow(Entitlements::Auditor::GitRepo).to receive(:new).with(logger, auditor2_cfg).and_return(auditor2)
  ...
  describe "#execute" do
    ...
    let(:auditor1) { instance_double(Entitlements::Auditor::GitRepo) }
    let(:auditor2) { instance_double(Entitlements::Auditor::GitRepo) }
